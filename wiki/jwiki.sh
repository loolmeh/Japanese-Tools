...
lookup()
{
  qurl="$url$query"
  result=$(
  curl -s "$qurl" | sed '/<p><b>/,/<table id="toc" class="toc">/!d' | grep '<p>' | w3m -dump -T text/html | sed ':a;N;$!ba;s/\n/ /g' | head -n 20
  )
} 
...
search()
{
  qurl="http://ja.wikipedia.org/w/index.php?title=Search&search=$query&fulltext=Search"
  result=$(
  curl -s "$qurl" | \
  sed '/table>/,//!d' | sed '/mw-search-results/,//!d' | w3m -dump -T text/html | sed 's/.*キロバイト.*//g;/^$/d' | \
  head -n 20 
  )
}
